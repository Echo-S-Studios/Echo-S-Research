...
$x=\pm1$, $256$ fail the trace-polynomial Sturm pattern $(1,0,5)$, and $46$
are reducible (none fail only trace-polynomial irreducibility); together
with the $37$ Salem classes, $39+256+46+37=378$ accounts for every
twist-class (ledger entry~W). A twist-class whose polynomial has a cyclotomic
factor is counted as \emph{reducible} regardless of a coincidental trace
pattern; the single class $x^{12}-x^{11}-x^{10}-x^{9}-x^{7}-x^{6}-x^{5}-x^{3}-x^{2}-x+1$,
which carries the factor $\Phi_{10}$, therefore sits in the reducible bucket
rather than among the trace-Sturm rejects.
\end{remark}
...
--- all twenty-six cross-engine items of ledger entry~M
(twenty-four signatures, two factorization checks).
\end{itemize}
...
$39/256/46/0/37$, sum $378$. Single-engine
