...
\small 19 session harnesses (v1.7--v2.1): 468 exact/computed checks\\
\small $+$ 67 certified guards $+$ 13 PSLQ corroborations $+$ 7 labelled walls, all exit 0\\
\small closure harness \texttt{nd\_softcheck.py}: 28/28, 565 forced objects, exit 0\\
\small finalization harness \texttt{bl\_softcheck.py}: 16/16 exact $+$ 7 walls LABELLED (\S\ref{sec:blockers}), exit 0}
\date{Version 2.1 --- July 15, 2026}
...
neither). With it the \emph{nineteen} session harnesses total $\mathbf{468}$
exact/computed checks $+$ $\mathbf{67}$ certified guards $+$ $\mathbf{13}$ PSLQ
corroborations $+$ $\mathbf{7}$ labelled walls (six after
Finding~\ref{find:blstale}), all exit 0.
...
P1$'$ cross-shell $\nu$ & \textsc{unfinished-computation} & within-shell decided; cross-shell mirrored-class coherence is a no-tie; labelled \hx{BL-G7} & the cross-shell coherence detector (tractable)\\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{P1\texorpdfstring{$'$}{'} cross-shell nu-criterion}\label{blk:p1cross}
\textbf{Status} (register): within-shell classification delivered (v1.8); the
cross-shell $\nu$-criterion half remains open (Remark~\ref{rem:p1scan}).
\textbf{Blocker:} \textsc{unfinished-computation}. \textbf{Forced fact:} the
within-shell coherence is decided by the bounded transport-orbit scan, but the
cross-shell mirrored-class coherence is left a no-tie (origin \texttt{rn} /
RN-3). \textbf{Missing object:} a cross-shell coherence detector --- an object
that is \emph{tractable in principle} and simply not yet executed, not a wall of
principle. Labelled \hx{BL-G7}: unlike every other item here, this one is a
finite computation awaiting completion.

\smallskip\noindent
\textbf{Closing.} The corpus's forced content is complete through v1.9, and v2.1
adds one item to it: the odd relational floor, proved $=2$
(Theorem~\ref{thm:odd2nd}). What remains falls into five of the six blocker
classes above --- the \textsc{named-lemma} class is now empty, and its single
occupant was discharged from inside, by an elementary argument requiring no
external theorem and no new axiom. \emph{Three are hard
walls internal to the axiom system} --- the two impossibilities (OP-RATE at
discrete resolution and OP-RADIUS ab initio) and the restatement wall (D2);
\emph{two are single declared atoms} (D4's cap-onset and orientation);
\emph{three are external tool-gaps} (the transcendence and algebraic-independence questions, blocked
respectively on a nonexistent transcendental-modulus theorem, on Schanuel's
conjecture, and on Kontsevich--Zagier period theory); and \emph{one is an
unfinished computation} (P1$'$ cross-shell). \emph{Eight rows, not nine.} No
remaining item is a mere oversight or an unattempted derivation. Each is a
precisely located wall, and for each this paper states the load-bearing forced
fact and the exact missing object rather than claiming a resolution --- and, per
the honesty gate, every \textsc{impossibility} is scoped relative to D1--D3 and
no external-open item is asserted to be resolved. One caution the ODD-2 closure
earns its own ledger: a blocker classification is a statement about the routes
in hand, and \S\ref{blk:odd2} sat at \textsc{named-lemma} for three versions on
the strength of a framing (Finding~\ref{find:sssframe}) rather than a theorem.
The remaining eight are argued from forced facts, not from framings --- but the
distinction is exactly what this section exists to keep visible.
