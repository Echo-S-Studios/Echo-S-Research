...
\small 22 session harnesses (v1.7--v2.6): 498 exact/computed checks\\
\small $+$ 70 certified guards $+$ 6 fail-first falsifiers $+$ 13 PSLQ $+$ 7 labelled walls, all exit 0\\
\small closure harnesses \texttt{nd\_softcheck.py} 28/28 (ODD-2), \texttt{xs\_softcheck.py} 10/10$+$3 (cross-shell), \texttt{ev\_softcheck.py} 16/16$+$6 (even floor $+$ $r{=}1$ peel), \texttt{ax\_softcheck.py} 4/4 ($C{=}4/9$ kill), exit 0\\
\small finalization harness \texttt{bl\_softcheck.py}: 16/16 exact $+$ 7 walls LABELLED (\S\ref{sec:blockers}; five active, \hx{BL-G4}/\hx{BL-G7} off the board), exit 0\\
\small register-motion harnesses (v2.7): \texttt{d4}/\texttt{d2}/\texttt{ul}/\texttt{db\_softcheck.py} $18$/$37$/$25$/$29$, \texttt{fc\_probe.py} $11$ (bounded), exit 0\\
\small open-tag resolution (v2.8): \texttt{pu\_softcheck.py} $22$ ($r{=}1$ floor), \texttt{ul\_closure\_check.py} $22$ (UL-1), \texttt{fc\_unbounded\_check.py} $24$ (door, all even $n$), \texttt{ws\_structure\_check.py} $23$ (within-shell), exit 0\\
\small door-traffic dichotomy $+$ degree-$10$ within-shell extension (v2.9): \texttt{open\_tags\_resolution\_check.py} $74$ (Thm~\ref{thm:doortraffic}; python-flint), exit 0}
\date{Version 2.9 --- July 16, 2026}
