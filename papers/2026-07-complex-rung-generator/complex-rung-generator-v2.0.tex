...
\small 18 session harnesses (v1.7--v2.0): 440 exact checks $+$ 67 certified guards $+$ 13 PSLQ corroborations $+$ 7 labelled walls, all exit 0\\
\small finalization harness \texttt{bl\_softcheck.py}: 16/16 exact $+$ 7 walls LABELLED (\S\ref{sec:blockers}), exit 0}
\date{Version 2.0 (final) --- July 15, 2026}
...
P3/P4 (odd floor $=2$) & \textsc{named-lemma} (ODD-2) & the $=2$ ceiling is forced \hx{BL-E1}; Smyth alone gives only $\mu_S{<}\gp{<}2$ \hx{BL-E2} & a Mahler-measure lower bound of $2$ in the charge-constrained regime\\[2pt]
...
\subsection{P3/P4: the odd relational floor equals two}\label{blk:odd2}
\textbf{Status} (register): \emph{bracketed, sharpened, and reduced to one named
lemma} (v1.9); computed $=2$ up to a logged enumeration box for $m=3,5,7,9$, and
P4 inherits P3 exactly (Proposition~\ref{prop:oddfloorred},
Remark~\ref{rem:p4transfer}). \textbf{Blocker:} \textsc{named-lemma} (ODD-2).
\textbf{Forced fact:} the $=2$ \emph{ceiling} is forced --- $\M(x^m-2)=2$ for odd
$m$ ($x^m-2$ is Eisenstein at $2$, all roots at modulus $2^{1/m}>1$, \hx{BL-E1})
--- but Smyth's theorem alone gives only $\mu_S<\gp<2$ (the plastic number
$\mu_S=1.3247$, \hx{BL-E2}), and the real-reciprocal sub-case is forced
$\ge\gp^2>2$ (\hx{BL-E3}); origin \texttt{of}. The residual lift $\mu_S\to2$ for
charge-admissible \emph{non-reciprocal} odd-charge objects is precisely the open
lemma ODD-2 (\hx{BL-G4}). \textbf{Missing object:} a Mahler-measure lower bound
of $2$ in that charge-constrained regime --- new number theory strengthening
Smyth. The $=2$ floor is therefore \emph{reduced to exactly ODD-2}, not proved;
the naive ``non-reciprocal $+$ Smyth forces $\ge2$'' idea is falsified.
...
\smallskip\noindent
\textbf{Closing.} The corpus's forced content is complete through v1.9. What
remains falls into exactly the six blocker classes above: \emph{three are hard
...
\emph{two are single declared atoms} (D4's cap-onset and orientation); \emph{one
is a single named number-theoretic lemma} (ODD-2); \emph{three are external
tool-gaps} (the transcendence and algebraic-independence questions, blocked
...
unfinished computation} (P1$'$ cross-shell). No remaining item is a mere
oversight or an unattempted derivation. Each is a precisely located wall, and for
each this paper states the load-bearing forced fact and the exact missing object
rather than claiming a resolution --- and, per the honesty gate, every
