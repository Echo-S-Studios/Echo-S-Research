...
\small 22 session harnesses (v1.7--v2.5): 498 exact/computed checks\\
...
\small finalization harness \texttt{bl\_softcheck.py}: 16/16 exact $+$ 7 walls LABELLED (\S\ref{sec:blockers}; five active after v2.2), exit 0}
\date{Version 2.5 --- July 16, 2026}
...
\hx{BL-G4} wall label are v2.0 artifacts, superseded and not re-run). One
...
Theorem~\ref{thm:odd2nd}. The harness is \emph{not} re-run in v2.1 and its
shipped pass line (\texttt{16/16 exact, 7 walls LABELLED}) is retained verbatim
as the v2.0 record; the row is kept in the \S\ref{sec:blockers} table, marked
...
inference that no further internal route existed. For reading purposes the
labelled-wall count is six, not seven.
\end{finding}
...
and \hx{BL-G4} is no longer a wall. The row and the harness output are retained
verbatim rather than rewritten --- the ledger is a record of what blocked what,
and a blocker that fell is part of that record (Finding~\ref{find:blstale}).
