...
W\tagO{}2 / D2 (\S\ref{blk:d2}) & internal --- drop-one model & a model of D1, D3 and the keystone algebra that \emph{violates} D2. If none exists, D2 is redundant given Theorem~\ref{thm:factorization}$+$D3 and the row \emph{retires}; if one exists, D2 is a genuine atom. A runnable audit, not yet run.\\[2pt]
...
means is therefore \emph{three named objects}: the $r=1$ Pisot-unit lemma
(Rem~\ref{rem:evenlocalize}), the apex-localized structure theorem
(\S\ref{blk:radab}), and the D2 drop-one model (\S\ref{blk:d2}). Two further items
want a scoped external theorem or a dynamics principle; three wait on named
conjectures that do not yet exist. Nothing here is an unattempted derivation ---
each is a precisely-shaped object, and three of them are internal.
