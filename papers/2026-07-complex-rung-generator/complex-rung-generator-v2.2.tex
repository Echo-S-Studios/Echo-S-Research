...
\small 20 session harnesses (v1.7--v2.2): 478 exact/computed checks\\
\small $+$ 70 certified guards $+$ 13 PSLQ corroborations $+$ 7 labelled walls, all exit 0\\
\small closure harnesses \texttt{nd\_softcheck.py} 28/28 (ODD-2) and \texttt{xs\_softcheck.py} 10/10$+$3 (cross-shell no-tie), exit 0\\
...
Theorem~\ref{thm:odd2nd}. With it the \emph{twenty} session harnesses total
$\mathbf{478}$ exact/computed checks $+$ $\mathbf{70}$ certified guards $+$
$\mathbf{13}$ PSLQ corroborations $+$ $\mathbf{7}$ labelled walls (five after
\hx{BL-G7} joins \hx{BL-G4} off the board), all exit 0.
...
\hx{BL-G7} is discharged; this was the sole \textsc{unfinished-computation}, and
that class is now \emph{empty}.
...
Kontsevich--Zagier period theory). \emph{Seven rows, not eight.} No remaining item
is a mere oversight or an unattempted derivation --- everything the corpus could
settle by its own means, it now has. Each survivor is a precisely located wall,
