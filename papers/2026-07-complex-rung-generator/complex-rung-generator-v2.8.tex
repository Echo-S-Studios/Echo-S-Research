...
the commutator door (external)}. This does not disturb the excision --- Salems
...
full-matrix-algebra requirement; the dichotomy's \emph{inadmissible-traffic} half
(whether every inadmissible object walks through the door) stays \tagO{}.
...
full-matrix requirement discharges while its traffic stays open; within-shell gains a
refutation and a mechanism theorem while its exhaustiveness stays \tagC{}. This is
\emph{not} ``everything closed''. No forced-content theorem is disturbed.
...
(\texttt{fc\_unbounded\_check}: $A_n=M_n(\Q)$), and the within-shell $p=s(x^k)$
conjecture is \emph{refuted} with its mechanism proved degree-free
(\texttt{ws\_structure\_check}). So the count of \emph{named objects the corpus must
still produce} drops to \emph{zero} --- carrying exactly two honest caveats: UL-1's
closure rests on the declared ambient, and the within-shell taxonomy
