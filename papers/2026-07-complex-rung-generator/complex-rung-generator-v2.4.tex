...
\small 22 session harnesses (v1.7--v2.4): 495 exact/computed checks\\
\small $+$ 70 certified guards $+$ 6 fail-first falsifiers $+$ 13 PSLQ $+$ 7 labelled walls, all exit 0\\
\small closure harnesses \texttt{nd\_softcheck.py} 28/28 (ODD-2), \texttt{xs\_softcheck.py} 10/10$+$3 (cross-shell), \texttt{ev\_softcheck.py} 13/13$+$6 (even floor), \texttt{ax\_softcheck.py} 4/4 ($C{=}4/9$ kill), exit 0\\
...
Proposition~\ref{prop:unimon} at the prime $3$. With them the \emph{twenty-two}
session harnesses total $\mathbf{495}$ exact/computed checks $+$ $\mathbf{70}$
certified guards $+$ $\mathbf{6}$ fail-first falsifiers $+$ $\mathbf{13}$ PSLQ
corroborations $+$ $\mathbf{7}$ labelled walls (five after \hx{BL-G7} joins
\hx{BL-G4} off the board), all exit 0.
