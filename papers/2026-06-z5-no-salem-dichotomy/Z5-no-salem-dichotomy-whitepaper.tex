...
that $\psi^5$ destroys. That gap is Schur--Siegel--Smyth trace--problem territory and is not closable
from scratch in--session. By the asymmetry rule, a clean window plus the four forced sub--results is
...
reciprocal charge--$5$ found; measures & $4$ objects; \ $\{1,\varphi^2,2+\sqrt3\}$ (all $=\Phi_5\times$ real recip.\ unit)\\
...
Residual $=$ non--reciprocal in $[\muS,2)$; trace--problem obligation & \open{} & Rem.~\ref{rem:notforced}\\
\bottomrule
\end{tabular}
\end{center}
