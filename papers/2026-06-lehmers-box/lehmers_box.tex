...
\end{array}
\]
totalling $27$. \computed{} {\small(\texttt{P2-UNIF-03}; Galois correspondence on the closure $K(i)$, $G\cong C_2\times C_2\times D_4$, the $27$ subgroups fixing a subfield of $K$ --- equivalently the $27$ subfields --- enumerated)}
\end{theorem}
...
\begin{lemma}[Shoda]\label{lem:shoda}
...
\begin{remark}[What is genuinely open, stated not hidden]\label{rem:residue}
Two fronts remain. \emph{(a)} The \emph{free} (non--self--action) commutator at unbounded size is the one door (Lemma~\ref{lem:shoda}); it is handled per instance by the Sturm--decidable flip--straddle (Lemma~\ref{lem:salemflip}) and the exact guard (Proposition~\ref{prop:guard}), and uniformly for the framework's actual commutator (Theorem~\ref{thm:uniform}), but not by a single uniform theorem covering \emph{every} free commutator. \openq{} \emph{(b)} Lehmer's problem \textbf{(P)} itself remains open in number theory, untouched. \openq{} Neither is concealed; both are named.
\end{remark}
...
Free commutator at unbounded size; Lehmer's problem & \openq & --- \\
\bottomrule
\end{tabular}
\end{center}
