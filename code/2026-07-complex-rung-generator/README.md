...
| `ev_softcheck.py` | v2.3: the EVEN relational floor M(α) ≥ φ (`thm:evenfloor`) — the even mirror of the odd descent with **Schinzel**'s totally-real bound M(N) ≥ φ^{e/2} in place of TP-2; EV-S the Schinzel battery (0 violations, tight at x²−x−1), EV-D the descent certificate, EV-F the parity-split floor scan (even-min φ, odd-min 2), EV-A the p₁=q₂ identity discharging `rem:mahlerfloor`. Six fail-first falsifiers | 13 + 6 fal. |
